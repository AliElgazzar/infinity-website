...
export function ProjectCard({ project, className, featured = false }: ProjectCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden image-shine">
        <Image
          src={project.image}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-navy via-navy/70 to-navy/15 transition duration-500 group-hover:via-navy/55" />
      <div
        aria-hidden="true"
        className="absolute inset-0 opacity-0 transition duration-500 group-hover:opacity-100"
      >
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom,rgba(242,140,40,0.25),transparent_55%)]" />
      </div>
      <div
        aria-hidden="true"
        className="absolute top-0 left-0 z-[1] h-full w-[3px] origin-top scale-y-35 bg-gradient-to-b from-orange to-electric transition duration-500 group-hover:scale-y-100"
      />
      <div className="relative z-[1] flex h-full flex-col justify-end p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center gap-2 font-mono-tech text-[0.62rem] uppercase">
          <span className="border border-white/20 bg-white/5 px-3 py-1 text-orange backdrop-blur-sm">
        ...
        <h3
          className={cn(
            "heading-card transition duration-300 group-hover:text-orange",
            featured ? "md:text-[2rem]" : "md:text-[1.75rem]",
          )}
        >
          {project.title}
        </h3>
